            ...
            {activeSection === 'history' && (
              <div className="space-y-6">
                ...
                {transactions.map(txn => (
                  <div key={txn.id} className="flex items-center justify-between p-6 rounded-2xl bg-slate-50 hover:bg-white  hover:border-slate-100 border-2 border-gray-100 transition">
                    <div className="flex items-center gap-5">
                      <div className={`p-4 rounded-2xl ${txn.status === 'Success' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                        {txn.status === 'Success' ? <FiCheckCircle /> : <FiAlertCircle />}
                      </div>
                      <div><p className="font-black text-slate-900">{txn.id}</p><p className="text-[11px] font-bold text-slate-400 uppercase">{txn.date}</p></div>
                    </div>
                    <div className="text-right"><p className="font-black text-slate-900">₹{txn.amount}</p></div>
                  </div>
                ))}
              </div>
            )}
