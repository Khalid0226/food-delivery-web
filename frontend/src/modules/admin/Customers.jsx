...
export default function Customers() {
    ...
    const deleteCustomer = async (email) => {
        try {
            const confirmDelete = window.confirm('Are you sure? This will remove the customer and their order history.');

            if (confirmDelete) {
                // Email ko safely encode karein
        ...
        <AdminLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        <div className="p-10 text-center font-bold text-slate-400">Loading Customers...</div>
                    ) : (
                                            ...
                                            <td className="px-6 py-4 text-right relative">
                                                ...
                                                {activeMenuId === cus.id && (
                                                    <div className="absolute right-6 mt-2 w-36 bg-white border border-slate-100 rounded-xl shadow-2xl z-20 overflow-hidden">
                                                        <button onClick={() => { navigate(`/admin/customer-profile/${cus.id}`); }} className="flex w-full items-center gap-2 px-4 py-2 hover:bg-slate-50 text-xs font-bold text-slate-700">
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/50">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(prev => prev + 1)} className="p-2 rounded-xl border bg-white hover:bg-slate-100 disabled:opacity-50"><FiChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
